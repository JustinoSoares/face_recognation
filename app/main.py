...
app = Flask(__name__)
socketio = SocketIO(app, cors_allowed_origins="*", async_mode='threading')  # Alternati
prisma = Prisma()
...
async def process_video():
    ...
    try:
        known_faces, alunos_map = await get_known_faces_from_db()
        while True:
            ...
            for (top, right, bottom, left), unknown_encoding in zip(face_locations, unknown_encodings):
                match_results = {}
                for known_encoding, name in known_faces:
                ...
                if label != "Desconhecido":
                    ...
                    print("Face desconhecida detectada.")
                # Desenha o retângulo e o rótulo no frame
            ...
            # Envia o frame processado ao cliente
